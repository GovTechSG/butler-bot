...
function promptTodayOrDateOption(roomSelectedId, query, hasPrevMsg) {
    ...
    var msg = 'When would you like to book the *' + roomlist[roomSelectedId] + '*?';
    if (hasPrevMsg == true) {
...
function promptTimeslotSelection(query, room, startDate){
	...
	cal_app.listEmptySlotsInDay(startDateStr, room)
        .then(function(jsonArr) {
            ...
            var msg = 'You have selected:\n*' + roomlist[room] + '* >> *' + startDate.getFormattedDate() + '*\n\nSelect a start time from ';
            if (startDate.getSimpleDate() == new Date().getSimpleDate()){
            	msg += 'Today (' + startDate.getFormattedDate() + ')';
            }else{
